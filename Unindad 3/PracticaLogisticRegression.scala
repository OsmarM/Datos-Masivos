...
//    - Cree una nueva clolumna llamada "Hour" del Timestamp conteniendo la  "Hour"

val logregdataall = (timedata.select(data("Clicked on Ad").as("label"), $"Daily Time Spent on Site", $"Age",
                    $"Area Income", $"Daily Internet Usage", $"Hour", $"Male"))


//logregdataall.select(logregdataall("Timestamp").as("Hour")).show()
